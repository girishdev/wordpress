			...
			<div class="wrapper">
				<div class="container">
					...
					if($installedUPCP){
						...
						$ajaxSearchType = get_theme_mod( 'upcp_theme_setting_header_ajax_search_type', 'off' );
						if ( get_theme_mod( 'upcp_theme_setting_header_search_type', 'upcp' ) == 'woocommerce' and function_exists('woocommerce_get_page_id')) {$Search_URL = get_permalink( woocommerce_get_page_id( 'shop' ) );}
						else {$Search_URL = get_theme_mod( 'upcp_theme_setting_catalog_url_text', $CatalogUrlDefault );}
						$Product_Class = get_theme_mod( 'upcp_theme_setting_header_search_type', 'upcp' );
						$AJAX_Catalogue_Search = get_option('EWD_UPCP_Theme_AJAX_Catalogue_Search');
						$AJAX_Search_Class = "";
						?>
						<form id="headerSearch" class="<?php echo $AJAX_Search_Class . ' ' . $Product_Class; ?>" method="post" action="<?php echo esc_url($Search_URL); ?>">
